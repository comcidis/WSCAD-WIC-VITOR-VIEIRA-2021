...
do
        ...
        APP="python3.8 floresta_Class_Numerica.py ${EXEMPLO[j]} ${FLOAT[l]} ${DASK[k]} ${ATRIBUTOS[m]}"
        $EXECUCAOPERF $COMANDO2 $APP &>Resultado_$[i]_${EXEMPLO[j]}kExemplos_FLOAT${FLOAT[l]}_DASK_${DASK[k]}_ATRIBUTOS_${ATRIBUTOS[m]}.txt
if [ -d "Resultados/${EXEMPLO[j]}kNumerica" ]
then
        mv Resultado_$[i]_${EXEMPLO[j]}kExemplos_FLOAT${FLOAT[l]}_DASK_${DASK[k]}_ATRIBUTOS_${ATRIBUTOS[m]}.txt Resultados/${EXEMPLO[j]}kNumerica
else
        mkdir -p Resultados/${EXEMPLO[j]}kNumerica
        mv Resultado_$[i]_${EXEMPLO[j]}kExemplos_FLOAT${FLOAT[l]}_DASK_${DASK[k]}_ATRIBUTOS_${ATRIBUTOS[m]}.txt Resultados/${EXEMPLO[j]}kNumerica
fi
done
done
